...
description: "Learn to use Ladybug's Rust API to analyze social network data with hands-on examples"
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

This tutorial will get you started using Ladybug's Rust API to analyze a dataset.
...
Create a new Cargo project with Ladybug as a dependency:

```bash
cargo new lbug_social_network
cd lbug_social_network
cargo add lbug
```
...
Before we can start querying the data, we need to create a new Ladybug database
...
`cargo run` will take some time as it [compiles Ladybug from scratch](https://docs.rs/lbug/latest/lbug/).
The second and subsequent runs should be faster.
```bash
$ cargo run --bin lbug_social_network
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 3m 18s
     Running `target/debug/lbug_social_network`
...
### Define schema
The first step in building a Ladybug graph is to define a schema. For our example dataset, we need the following node and relationships tables:
```rs
conn.query(
...
Run `create_db.rs` to create the Ladybug database and import the CSV files:
```bash
$ cargo run --bin create_db
id|name|type|database name|comment
0|User|NODE|local(lbug)|
1|Post|NODE|local(lbug)|
3|FOLLOWS|REL|local(lbug)|
...
## Summary
In this tutorial, we've shown how to use Ladybug's Rust API to import CSV data as a graph and query it using Cypher.

You're now ready to import your own datasets into Ladybug and query them using Ladybug's Rust API!
