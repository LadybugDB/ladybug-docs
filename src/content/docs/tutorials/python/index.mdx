...
description: "Learn to use Ladybug's Python API to analyze social network data with hands-on examples"
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

This tutorial will get you started using Ladybug's Python API to analyze a dataset.
...
a virtual environment and install Ladybug via `pip`. Refer to this [page](/installation/#python)
...
Before we can start querying the data, we need to create a new Ladybug database and import the CSV files.
...
- The `create_db.py` script will be used to create the Ladybug database and import the CSV files.
...
## Create the database
Clear out the contents of `src/create_db.py` and import Ladybug:
```py
import lbug
```
...
```py
def main() -> None:
    db = lbug.Database("social_network.lbug")
...
### Running the code
Run `create_db.py` to create the Ladybug database and import the CSV files:
```bash
...
## Summary
In this tutorial, we've shown how to use Ladybug's Python API to import CSV data as a graph and query it using Cypher.

You're now ready to import your own datasets into Ladybug and query them usin Ladybug's Python API!
