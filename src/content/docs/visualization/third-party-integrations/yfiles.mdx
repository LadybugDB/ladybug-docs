---
title: yFiles Jupyter Graphs for Ladybug
---
...
use cases, allowing users working in Python to visualize Ladybug graphs in their Jupyter notebooks. See
...
uv init
uv add ladybug yfiles-jupyter-graphs-for-kuzu
```
...
```bash
# Install lbug as a pre-requisite
pip install ladybug yfiles-jupyter-graphs-for-kuzu
```
...
It uses Ladybug's Python API under the hood to fetch the data as Python objects, which can be slow for
...
Ladybug stores all relationships as directed, but the yFiles Jupyter Graphs widget allows you to
