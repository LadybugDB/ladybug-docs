---
title: Python API
description: "Use Ladybug embedded graph database in Python applications with the official Python API"
---

import { LinkCard } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

Ladybug provides a Python package that you can install via PyPI. A full list of the available functions and classes
...
Ladybug provides both a synchronous and an asynchronous Python API. The synchronous API is the default
and is more convenient to use.

Download the [CSV files](https://github.com/LadybugDB/lbug/tree/master/dataset/demo-db/csv) used in the examples below:
```bash
mkdir ./data/
...
The asynchronous API is useful for running Ladybug in an async context,
such as in web frameworks like FastAPI or cases where you need to concurrently run queries in Ladybug.
...
In Python, Ladybug supports the use of Pandas and Polars DataFrames, as well as PyArrow Tables. This
...
You can scan a Pandas DataFrame, Polars DataFrame, or PyArrow Table in Ladybug using the `LOAD FROM` clause.
Scanning a DataFrame or Table does *not* copy the data into Ladybug, it only reads the data.
...
This section summarizes the type notation used in Ladybug's Python API. Below is a table from Python
types to a Ladybug `LogicalTypeID`, which will be used to infer types via Python type annotations.

|Python type|Ladybug `LogicalTypeID`|
|---|---|
|`bool`|`BOOL`|
|`int`|`INT64`|
|`float`|`DOUBLE`|
...
Ladybug's Python API also supports the registration of User Defined Functions (UDFs),
allowing you to define custom functions in Python and use them in your Cypher queries. This can allow
you to quickly extend Ladybug with new functions you need in your Python applications.

:::caution[Note]
There are performance implications of writing UDFs in Python. Before writing your own UDF, it's
recommended to check if an equivalent Cypher function in Ladybug exists, as native functions run faster
than UDFs.
:::
...
return value in Ladybug, prior to registering the UDF. 
...
for the parameters which are then manually casted to the respective Ladybug type.
