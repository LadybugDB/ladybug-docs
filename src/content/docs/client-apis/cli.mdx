...
Ladybug provides a CLI shell through which you can issue Cypher queries or `CALL` commands to query
...
Ladybug supports on-disk, as well as in-memory databases. The functionality of the CLI is identical for both on-disk and in-memory modes.

### On-disk database

You can create an on-disk database in the CLI by specifying a path after
the `lbug` command in the terminal. This opens the database on-disk, in read-write mode, and the
...
Alternatively, you can create an in-memory database by omitting the path entirely, and just calling `lbug`
from your terminal:
...
section. List all available shell commands by running `lbug -h`.
...
$ lbug -h
  ...
  OPTIONS:
      ...
      databasePath                      Path to the database. If not given or
      ...
      --maxdbsize=[max_db_size]         Maximum size of the database in bytes
      ...
      -b, --no_progress_bar,
      --noprogressbar                   Disable query progress bar
      -w, --ignore_wal_replay_errors    By default something goes wrong when
...
By default, Ladybug's CLI is launched with a maximum buffer pool size of 80% of the available memory.
If you want to launch the Ladybug CLI with a different buffer pool size, you can do so by setting the
...
```bash
lbug --defaultbpsize 4096
```


#### `:help`
Show built-in command list within the Ladybug shell.
...
lbug> :help
    ...
    Note: you can change and see several system configurations, such as num-threads,
          ...
          See: [cypher config](/cypher/configuration)
```
...
#### `:schema`
Print the database schema, showing all node tables, rel tables, and their properties.
...
lbug> :mode json
mode set as json
lbug> CREATE NODE TABLE Person (name STRING PRIMARY KEY, age INT64);
