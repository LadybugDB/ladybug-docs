...
std::vector<std::shared_ptr<lbug::processor::FlatTuple>> tuples;
...
Ladybug provides two interfaces that enable you to define your own custom scalar and vectorized functions.

### Scalar functions

This API allows user to register ordinary scalar functions which are defined in c++ and use them as Ladybug built-in functions in a query. UDF functions are as efficient as the built-in functions.
Since two c++ primitive types may map to the same Cypher data types(e.g. `int32` can map to both `INT32` or `DATE` in Ladybug), Ladybug provides two overloaded APIs which can eliminate the ambiguity in datatype mapping.

#### Option 1: Scalar function with automatic inference

Create a scalar function by automatically inferring the parameter and result type in Ladybug.
...
* `ARGS`: are the type of the arguments in c++, Ladybug currently support UDF functions with up to 3 parameters.

Parameters:
* `name`: the name of the function to be created in Ladybug (note: function name must be unique).
...
Due to its columnar storage and architecture decisions, Ladybug can execute functions on input data in an
efficient and vectorized manner. In addition to creating scalar UDFs, Ladybug also provides support for
...
#### Vector types in Ladybug

The first type of vector that Ladybug supports is flat vectors.
...
Create a vectorized function by automatically inferring the parameter and result type in Ladybug.
```cpp
template<typename TR, typename... Args>
...
// Note: Users can utilize Ladybug pre-defined FunctionExecutors(UnaryFunctionExecutor, BinaryFunctionExecutor, TernaryFunctionExecutor) to execute operations on parameters without writing code to retrieve the valueVectors.
static void addDate(
    const std::vector<std::shared_ptr<ValueVector>>& parameters, ValueVector& result) {
