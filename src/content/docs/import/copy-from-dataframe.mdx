...
description: Import data directly from Pandas and Polars DataFrames or PyArrow tables into Ladybug using COPY FROM commands.
---

You can copy from Pandas/Polars dataframes or in-memory Arrow tables directly into Ladybug using the `COPY FROM` command.
This is useful when you are performing data transformations with these libraries and want to quickly ingest
the prepared data into Ladybug.

There are two different ways in Ladybug to import dataframes:
...
because Ladybug uses a strictly-typed schema.
:::
...
You can directly copy from a Pandas dataframe into Ladybug. Both numpy-backed and PyArrow-backed Pandas
...
You can utilize an existing in-memory PyArrow table to copy data directly into Ladybug.
...
We can now scan the DataFrame using the `LOAD FROM` clause in Ladybug in combination with `UNWIND` to
...
aligns well with Ladybug's type system.

#### Define Ladybug schema
...
The above code creates the following tables in Ladybug:
...
Let's proceed to copy the data from Polars into the Ladybug tables.
...
data type of `age` was inferred in Polars as `i8` (signed 8-bit integer), but the Ladybug schema requires
...
That's it! We have successfully copied the relevant data into our Ladybug graph.
...
the data using Polars dataframes and Ladybug. Happy querying!
