...
description: Bulk import data from CSV files into Ladybug using COPY FROM with extensive configuration options and error handling.
---
...
- **Wrap strings inside quotes:** Ladybug will accept strings in string columns both with and without quotes, though it's recommended to wrap strings in quotes to avoid any ambiguity with delimiters.
- **Avoid leading and trailing spaces**: As per the CSV standard, Ladybug does not ignore leading and trailing spaces (e.g., if you input `   213   ` for
...
Ladybug can read multiple files with the same structure, consolidating their data into a single node or relationship table.
...
```
Then, Ladybug will use the default values of `QUOTE` and `ESCAPE`, which are `"` and `\` respectively (and use
...
Ladybug parses the first line of the CSV into columns and checks if each column can be cast to the data type of the target column in the node or rel table that is being copied into.
