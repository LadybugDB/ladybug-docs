...
description: Bulk import data from JSON files into Ladybug node and relationship tables using the JSON extension.
---

## JSON files

You can copy from JSON files directly into Ladybug using the `COPY FROM` command. To use this feature,
...
Like for CSV files, Ladybug can skip rows when some types of errors are encountered when importing from JSON.
