...
description: "Migrate your Neo4j graph database data directly into Ladybug"
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

The `neo4j` extension allows you to migrate your Neo4j data into a Ladybug database.
It provides the following function:

- `NEO4J_MIGRATE`: Migrate data from Neo4j to Ladybug

:::note[Note]
* Multi-labelled nodes in Neo4j cannot be migrated as Ladybug doesn't support them yet.
* Nodes and relationships without any labels in Neo4j will be skipped during migration.
* Any property with a data type that is not natively supported in Ladybug will be imported into Ladybug as a `STRING`.
:::
...
The following section shows how to migrate _labels_ from Neo4j to _tables_ in Ladybug.

### Importing data from Neo4j

You can use the `NEO4J_MIGRATE` function to import data from Neo4j into Ladybug:
...
- `PASSWORD`: Neo4j password. Leave blank if not needed.
  - Type: `STRING`
- `NODE_LABELS`: A list of node labels to migrate to Ladybug. Use `["*"]` to import all nodes with labels.
  - Type: `STRING[]`
- `REL_LABELS`: A list of relationship labels to migrate to Ladybug. Use `["*"]` to import all relationships with labels.
  - Type: `STRING[]`
...
You can use `SHOW_TABLES()` to verify that the tables have been imported into Ladybug:
```cypher
...
The data from Neo4j is now available in Ladybug and you can start querying them.
