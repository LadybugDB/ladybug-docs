---
title: "S3 storage"
description: "Read, write or glob files hosted on Amazon S3"
---
...
[CALL](/cypher/configuration) statements in Cypher.
```cypher
...
- `s3_url_style` defaults to virtual-hosted-style. Some services (for example, MinIO in its default configuration) require path-style URLs — set `s3_url_style='path'` in that case.
