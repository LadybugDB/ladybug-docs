...
for accelerating similarity search over vector embeddings (32-bit and 64-bit float arrays) stored in Ladybug.
...
library, such as `sentence_transformers` in Python to create the embeddings. Alternatively, you can use Ladybug's [`llm`](/extensions/llm)
...
ingested into the Ladybug database.
...
Ladybug allows you to combine vector search with filter predicates by using [projected graphs](/extensions/algo#projected-graphs).
