...
The `duckdb` extension allows you to [attach](/extensions/attach/rdbms) DuckDB databases to Ladybug.
...
- `alias`: Database alias to use in Ladybug. If not provided, the database name from DuckDB will be used.
...
- `schema`: The name of the schema in DuckDB to attach. Ladybug attaches to the `main` schema of DuckDB by default.
...
### DuckDB to Ladybug type mapping
