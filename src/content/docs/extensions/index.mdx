---
title: Extensions
description: Dynamically extend Ladybug's capabilities with official extensions for JSON, vector search, full-text search, and more.
---

Ladybug has an extension API that is designed to dynamically extend its capabilities. Extensions allow
...
|----------|----------|
| [adbc](/extensions/attach/adbc) | Scan data from any ADBC-compatible database (DuckDB, PostgreSQL, SQLite, Snowflake, etc.) |
...
| [httpfs](/extensions/httpfs) | Read and write files over HTTP(S) protocol. Also used for [attaching a remote Ladybug database](/extensions/attach/lbug). |
...
| [neo4j](/extensions/neo4j) | Migrate data from Neo4j into Ladybug |
...
```cypher
LOAD EXTENSION 'extension/custom_json/build/libjson.lbug_extension';
```
...
```
:::note[Note]
Uninstalling a currently loaded extension won't affect the running database instance, but you won't be able to load it again in a new Ladybug instance until it's reinstalled.
:::
