...
description: Extension for running native graph algorithms in Ladybug
---

The `algo` extension allows you to run common graph algorithms such as PageRank,
Connected Components, and Louvain on the graph stored in Ladybug. The algorithms are exposed as
Cypher functions and execute directly inside Ladybug.
...
The graph algorithms run on _projected graphs_ instead of operating directly on Ladybug database tables.
...
Ladybug does not materialize projected graphs in memory, and the corresponding data
...
In Ladybug, both the base graph and projected graphs are directed. For algorithms that are only
